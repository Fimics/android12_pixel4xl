...
final class UserManagerWrapper extends ServiceManagerWrapper<UserManager> {

    private static final String TAG = UserManagerWrapper.class.getSimpleName();

    private static final HashMap<Context, UserManager> sSpies = new HashMap<>();

    @Override
    UserManager getWrapper(Context context, UserManager manager, Answer<?> answer) {
        int userId = context.getUserId();
        UserManager spy = sSpies.get(context);
        if (spy != null) {
